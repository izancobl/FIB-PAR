...
void mandelbrot(int height, int width, double real_min, double imag_min,
                double scale_real, double scale_imag, int maxiter, int **output, int numtasks) {
    ...
    for (int row = 0; row < height; ++row) {
      #pragma omp taskloop num_tasks(numtasks) firstprivate(row)
...
int main(int argc, char *argv[]) {
    ...
    mandelbrot(height, width, real_min, imag_min, scale_real, scale_imag, maxiter, output, numtasks);
